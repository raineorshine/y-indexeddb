(function () {
   ...
   * @param {string} name
   */
  const keyRangeIndexAll = name => createIDBKeyRangeBound([name], [name, []], false, false);
   ...
   * @param {boolean} lowerOpen
   */
  const keyRangeIndexLowerBound = (name, lower, lowerOpen) => createIDBKeyRangeBound([name, lower], [name, []], lowerOpen, false);
   ...
   * @param {boolean} upperOpen
   */
  const keyRangeIndexUpperBound = (name, upper, upperOpen) => createIDBKeyRangeBound([name], [name, upper], false, upperOpen);
   ...
   */
  const fetchUpdates = async (idbPersistence, beforeApplyUpdatesCallback = () => {}, afterApplyUpdatesCallback = () => {}) => {
    const [updatesStore] = transact$1(/** @type {IDBDatabase} */ (dbcached), [updatesStoreName]); // , 'readonly')
    const updatesIndex = updatesStore.index('name,id');
    const updates = await rtop(updatesIndex.getAll(keyRangeIndexLowerBound(idbPersistence.name, idbPersistence._dbref, false)));
    if (!idbPersistence._destroyed) {
      beforeApplyUpdatesCallback(updatesStore);
      transact(idbPersistence.doc, () => {
        updates.forEach((/** @type {{ update: Uint8Array }} */record) => applyUpdate(idbPersistence.doc, record.update));
      }, idbPersistence, false);
      afterApplyUpdatesCallback(updatesStore);

      /** @type {[string, number] | undefined} */
      const nameKeyPair = await getLastKey(/** @type {any} */(updatesIndex), keyRangeIndexAll(idbPersistence.name));
      if (!nameKeyPair) return updatesStore
      idbPersistence._dbref = nameKeyPair[1] + 1;

      const count = await rtop(updatesIndex.count(keyRangeIndexAll(idbPersistence.name)));
   ...
   */
  const storeState = async (idbPersistence, forceStore = true) => {
    const updatesStore = await fetchUpdates(idbPersistence);
    if (forceStore || idbPersistence._dbsize >= PREFERRED_TRIM_SIZE) {
      await rtop(updatesStore.add({
        name: idbPersistence.name,
        update: encodeStateAsUpdate(idbPersistence.doc)
      }));
      const updatesIndex = updatesStore.index('name,id');
      // resolve when transaction auto-commits
      const keys = await rtop(updatesIndex.getAllKeys(keyRangeIndexUpperBound(idbPersistence.name, idbPersistence._dbref, true)));
  ...
  const clearDocument = async (name) => {
    const db = await (dbpromise || openDB(dbname, () => {}));
    return new Promise((resolve, reject) => {
      ...
      rtop(updatesIndex.getAllKeys(keyRangeIndexAll(name))).then(keys => {
        keys.forEach((/** @type {number} */key) => updatesStore.delete(key));
      });
    })
  };
   ...
   * @return {Promise<IDBDatabase>}
   */
  const openDBWithUpgradeTransaction = (name, initDB) => new Promise((resolve, reject) => {
    // eslint-disable-next-line
    const request = indexedDB.open(name);
    /**
     * @param {any} event
  ...
  class IndexeddbPersistence extends Observable {
    ...
    constructor (name, doc) {
      ...
      this.whenSynced = dbpromise.then(() => {
        ...
        const beforeApplyUpdatesCallback = (updatesStore) => {
          ...
          return update.length > 2
            ? rtop(updatesStore.add({ name: this.name, update }))
            : null
        };
      ...
      this._storeUpdate = (update, origin) => {
        if (origin !== this && this.created) {
          const [updatesStore] = transact$1(/** @type {IDBDatabase} */ (dbcached), [updatesStoreName]);
          rtop(updatesStore.add({ name: this.name, update }));
          this._dbsize++;
    ...
    async get (key) {
      ...
      const { value } = await rtop(custom.get([this.name, key]));
      return value
    }
    ...
    async set (key, value) {
      ...
      return rtop(custom.put({ name: this.name, value }, [this.name, key]))
    }
    ...
    async del (key) {
      const db = await (/** @type {Promise<IDBDatabase>} */(dbpromise));
      const [custom] = transact$1(db, [customStoreName]);
      return del(custom, [this.name, key])
    }
  }
  ...
  */
   ...
   * @param {t.TestCase} tc
   */
  const testPerf = async tc => {
    await measureTimeAsync('time to create a y-indexeddb instance', async () => {
      const ydoc = new Doc();
   ...
   * @param {t.TestCase} tc
   */
  const testMultipleDocs = async tc => {
   ...
   * @param {t.TestCase} tc
   */
  const testClearDataSingleDoc = async tc => {
    ...
    const db = await openDB('y-indexeddb', () => {});
    const [customStore, updatesStore] = transact$1(db, ['custom', 'updates']);
    const updates = await getAll(updatesStore, createIDBKeyRangeLowerBound(0, false));
    assert(updates.length === 0);

    // persistence1 should have no custom values
    const custom = await getAll(customStore, createIDBKeyRangeLowerBound(0, false));
   ...
   * @param {t.TestCase} tc
   */
  const testClearDocumentSingleDoc = async tc => {
    // this document will be cleared
    const doc1 = new Doc();
    const persistence1 = new IndexeddbPersistence(tc.testName, doc1);
    persistence1.set('a', 4);
   ...
   * @param {t.TestCase} tc
   */
  const testClearDataMultipleDocs = async tc => {
    // this document will be cleared
    const doc1 = new Doc();
    const arr1 = doc1.getArray('t');
    arr1.insert(0, [1]);
    ...
    const db = await openDB('y-indexeddb', () => {});
    const [customStore, updatesStore] = transact$1(db, ['custom', 'updates']);
    const updates = await getAll(updatesStore);
    assert(updates.every(({ name }) => name !== 'doc1'));

    // persistence1 should have no custom values
    const custom = await getAll(customStore);
    assert(custom.every(({ name }) => name !== 'doc1'));
   ...
   * @param {t.TestCase} tc
   */
  const testClearDocumentMultipleDocs = async tc => {
    // this document will be cleared
    const doc1 = new Doc();
    const persistence1 = new IndexeddbPersistence('doc1', doc1);
    persistence1.set('a', 4);
   ...
   * @param {t.TestCase} tc
   */
  const testClearDocumentAndReopen = async tc => {
    // this document will be cleared
    const doc1 = new Doc();
    const persistence1 = new IndexeddbPersistence(tc.testName, doc1);
    ...
    const db = await openDB('y-indexeddb', () => {});
    assert(db);
    assert(db.objectStoreNames.length === 2);
  ...
  var indexeddb = /*#__PURE__*/Object.freeze({
    ...
    testMultipleDocs: testMultipleDocs,
    testClearDataSingleDoc: testClearDataSingleDoc,
    testClearDocumentSingleDoc: testClearDocumentSingleDoc,
    testClearDataMultipleDocs: testClearDataMultipleDocs,
    testClearDocumentMultipleDocs: testClearDocumentMultipleDocs,
    testClearDocumentAndReopen: testClearDocumentAndReopen
  });
